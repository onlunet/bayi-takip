...
import {
  ...
  hasUserAuthCredential,
  upsertUserAuthCredential,
  verifyAdminAuthToken,
  verifyUserAuthCredential
...
function readAdminToken(request: any) {
  const fromHeader = readHeader(request.headers["x-admin-token"] as string | string[] | undefined);
  if (fromHeader) return fromHeader.trim();

  const authorization = readHeader(request.headers.authorization as string | string[] | undefined);
...
async function resolveAdminTokenUser(request: any) {
  ...
  if ((payload.companyId ?? null) !== userCompanyId) return null;

  return user;
}

async function resolveApiKeyUser(request: any) {
  const suppliedUserKey = readHeader(request.headers["x-api-key"] as string | string[] | undefined);
  if (!suppliedUserKey) return null;
...
async function resolveResourceCompanyScope(
  ...
  if (pathMatches(lowerPath, "/integrations") && id) {
    if (pathMatches(lowerPath, "/integrations/jobs")) {
      ...
      if (!job) return { companyId: null, notFound: true };
...
app.post("/admin/auth/login", async (request, reply) => {
  ...
  if (!hasCredential) {
    await upsertUserAuthCredential(prisma as any, user.id, data.password);
  }

  const role = user.role as "OWNER" | "ADMIN" | "WAREHOUSE" | "ACCOUNTING";
...
async function sendAuthMeResponse(request: any, reply: any) {
  const authRequired = await shouldRequireAdminAuth();
