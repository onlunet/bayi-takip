...
export async function registerStockMovementRoutes(app: FastifyInstance) {
  ...
  app.delete("/stock-movements/:id", async (request, reply) => {
    ...
    const isProtectedReference =
      movement.referenceType === "ORDER" || movement.referenceType === "DISPATCH";

    if (isReferenced && isProtectedReference && !force) {
      reply.status(400).send({
        message:
          "Bu hareket siparis/sevkiyat kaydina bagli. Once ilgili siparis veya sevkiyat kaydini duzeltin."
      });
      return;
    }
