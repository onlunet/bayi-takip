...
async function ensureLocation(type: "WAREHOUSE" | "DEALER", companyId: string, referenceId: string) {
...
async function applyDispatchEffects(
...
function calculateItemsTotal(
  items: Array<{ quantity: number | string | Prisma.Decimal; price: number | string | Prisma.Decimal }>
) {
...
async function getDealerRiskSnapshot(
  dealerId: string,
...
export async function registerDispatchRoutes(app: FastifyInstance) {
  app.post("/dispatches", async (request, reply) => {
    ...
    const dispatch = await prisma.$transaction(async (tx) => {
      ...
      if (created.status === "APPROVED") {
        ...
        const totalAmount = created.items.reduce(
          (sum, item) => sum.plus(item.quantity.mul(item.price)),
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (nextStatus === "APPROVED") {
        ...
        const totalAmount = nextItems.reduce(
          (sum, item) =>
            sum.plus(new Prisma.Decimal(item.quantity).mul(new Prisma.Decimal(item.price))),
          new Prisma.Decimal(0)
        );
  ...
  app.post("/dispatches/:id/approve", async (request, reply) => {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const totalAmount = dispatch.items.reduce(
        (sum, item) => sum.plus(item.quantity.mul(item.price)),
        new Prisma.Decimal(0)
      );
