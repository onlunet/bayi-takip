...
async function getOrCreateDealerLocationTx(
...
function canTransitionOrderStatus(fromStatus: string, toStatus: string) {
  if (fromStatus === toStatus) return true;
  const allowed = ORDER_TRANSITIONS[fromStatus] ?? [];
  return allowed.includes(toStatus);
}

function isRecoverableOrdersQueryError(error: unknown) {
...
export async function registerOrderRoutes(app: FastifyInstance) {
  ...
  app.patch("/orders/:id/status", async (request, reply) => {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (requiresStockReversal) {
        ...
        if (!exists) {
          const dealerLocation = await getOrCreateDealerLocationTx(
            tx,
            existing.companyId,
            existing.dealerId
          );
  ...
  app.get("/orders", async (request, reply) => {
    ...
    try {
      ...
      if (!isRecoverableOrdersQueryError(error)) throw error;
    ...
    >(
      `SELECT
       ...
       ORDER BY ${orderColumns.has("createdAt") ? `o."createdAt"` : `o."id"`} DESC`,
      ...params
    );
