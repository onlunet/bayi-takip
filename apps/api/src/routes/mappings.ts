...
export async function registerMappingRoutes(app: FastifyInstance) {
  ...
  app.post("/mappings/import", async (request, reply) => {
    ...
    const errors: Array<{ row: number; reason: string; externalProductId: string; externalVariantId?: string }> = [];
    let success = 0;
  ...
  app.get("/mappings", async (request, reply) => {
    ...
    const platform = (request.query as any).platform as "WOO" | "SHOPIFY" | "MARKETPLACE" | "OTHER" | undefined;
