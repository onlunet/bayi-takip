...
export async function registerUserRoutes(app: FastifyInstance) {
  app.post("/users", async (request, reply) => {
    ...
    if (role === "DEALER") {
      await upsertDealerAuthCredential(prisma as any, user.id, data.password);
    } else {
  ...
  app.patch("/users/:id", async (request, reply) => {
    ...
    if (password) {
      await ensureDealerMembershipTables();
      const effectiveRole = (data.role ?? user.role) as string;
      if (effectiveRole === "DEALER") {
        await upsertDealerAuthCredential(prisma as any, user.id, password);
      } else {
        await upsertUserAuthCredential(prisma as any, user.id, password);
      }
    }
