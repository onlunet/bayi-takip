...
export type RiskLedgerEntry = {
  type: LedgerEntryType;
  amount: number;
  date: Date | string;
};

export type DealerRiskPolicy = {
  creditLimit: number;
  paymentTerms: number;
};

export type DealerRiskSnapshot = {
...
export function calculateDealerRisk(
  entries: RiskLedgerEntry[],
  policy: DealerRiskPolicy,
  additionalDebtAmount = 0,
  now: Date = new Date()
): DealerRiskSnapshot {
  ...
  let oldestOpenDebtDate: Date | null = null;
  ...
  for (const entry of normalizedEntries) {
    ...
    if (runningBalance <= 0 && entry.amount > 0) {
      oldestOpenDebtDate = entry.date as Date;
    }
    runningBalance += entry.amount;
  }
...
export function buildRiskBlockMessage(snapshot: DealerRiskSnapshot) {
  if (snapshot.limitExceeded && snapshot.overdue) {
    return `Cari risk limiti ve vade asimi nedeniyle islem engellendi. Limit: ${snapshot.creditLimit.toFixed(
      2
    )}, Projeksiyon: ${snapshot.projectedBalance.toFixed(2)}, Gecikme: ${snapshot.overdueDays} gun.`;
  }

  if (snapshot.limitExceeded) {
    return `Cari risk limiti asiliyor. Limit: ${snapshot.creditLimit.toFixed(
      2
    )}, Projeksiyon: ${snapshot.projectedBalance.toFixed(2)}.`;
  }
