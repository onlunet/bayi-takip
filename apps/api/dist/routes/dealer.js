...
import { createDealerAuthToken, ensureDealerMembershipTables, hasDealerAuthCredential, verifyDealerAuthCredential, verifyDealerAuthToken } from "../lib/dealerMembership.js";
...
function isMissingStorefrontKeyColumn(error) {
...
async function readLegacyDealerSettingsByDealerId(dealerId) {
    const rows = await prisma.$queryRawUnsafe(`SELECT "id", "dealerId", "marginPercent", "roundingType", "apiKey", "createdAt", "updatedAt"
     FROM "DealerSettings"
     WHERE "dealerId" = $1
     LIMIT 1`, dealerId);
    return rows[0] ?? null;
}
async function readLegacyDealerSettingsByApiKey(apiKey) {
    const rows = await prisma.$queryRawUnsafe(`SELECT "id", "dealerId", "marginPercent", "roundingType", "apiKey", "createdAt", "updatedAt"
     FROM "DealerSettings"
     WHERE "apiKey" = $1
     LIMIT 1`, apiKey);
...
async function upsertLegacyDealerSettings(dealerId, data) {
    const existing = await readLegacyDealerSettingsByDealerId(dealerId);
    if (existing) {
        const marginPercent = data.marginPercent !== undefined ? data.marginPercent : Number(existing.marginPercent ?? 0);
        const roundingType = data.roundingType ?? String(existing.roundingType ?? "NONE");
        const updated = await prisma.$queryRawUnsafe(`UPDATE "DealerSettings"
       SET "marginPercent" = $1,
           "roundingType" = $2::"RoundingType",
           "updatedAt" = CURRENT_TIMESTAMP
       WHERE "dealerId" = $3
       RETURNING "id", "dealerId", "marginPercent", "roundingType", "apiKey", "createdAt", "updatedAt"`, marginPercent, roundingType, dealerId);
        return updated[0] ?? null;
    }
    const created = await prisma.$queryRawUnsafe(`INSERT INTO "DealerSettings"
     ...
     RETURNING "id", "dealerId", "marginPercent", "roundingType", "apiKey", "createdAt", "updatedAt"`, `legacy_${Math.random().toString(36).slice(2)}${Date.now().toString(36)}`, dealerId, data.marginPercent ?? 0, data.roundingType ?? "NONE", generateKey());
    return created[0] ?? null;
}
async function getDealerSettingsByDealerId(dealerId) {
    let settings = null;
    try {
...
function readDealerToken(request) {
...
async function getDealerByApiKey(request, reply) {
...
async function getDealerByStorefrontKey(storefrontKey) {
    try {
...
function applyRounding(price, roundingType) {
    ...
    if (roundingType === "ROUND_90")
        return new Prisma.Decimal(Math.floor(value) + 0.9);
    if (roundingType === "ROUND_99")
        return new Prisma.Decimal(Math.floor(value) + 0.99);
    if (roundingType === "ROUND_50")
        return new Prisma.Decimal(Math.floor(value) + 0.5);
    if (roundingType === "INTEGER")
...
function toXml(items) {
    ...
    items.forEach((item) => {
        ...
        if (item.sku)
            lines.push(`    <sku>${escape(item.sku)}</sku>`);
        lines.push(`    <unit>${escape(item.unit)}</unit>`);
        if (item.imageUrl)
            lines.push(`    <image>${escape(item.imageUrl)}</image>`);
        ...
        if (item.variants && item.variants.length) {
            ...
            item.variants.forEach((variant) => {
                ...
                if (variant.sku)
                    lines.push(`    <sku>${escape(variant.sku)}</sku>`);
...
async function getPriceMap(dealerId) {
...
async function getDealerRiskSnapshot(dealerId, additionalDebtAmount = 0) {
    const dealer = await prisma.dealer.findUnique({
        where: { id: dealerId },
        select: {
            id: true,
    ...
    const risk = calculateDealerRisk(entries.map((entry) => ({
        type: entry.type,
...
async function createDealerOrderFromItems(dealer, settings, inputItems, options) {
...
function toDealerOrderHistoryRows(orders) {
    return orders.map((order) => {
        const itemsSummary = order.items
            ...
            .join(", ");
        const customerDisplay = order.customer?.name ??
            order.customerName ??
...
export async function registerDealerRoutes(app) {
    ...
    app.get("/dealers/risk-summary", async (request, reply) => {
        ...
        const rows = dealers.map((dealer) => {
            const risk = calculateDealerRisk(byDealerId.get(dealer.id) ?? [], {
                creditLimit: Number(dealer.creditLimit ?? 0),
                paymentTerms: Number(dealer.paymentTerms ?? 0)
            }, 0);
            return {
    ...
    app.get("/dealers/:id/risk", async (request, reply) => {
        const dealerId = request.params.id;
        const additionalDebt = Number(request.query.additionalDebt ?? 0);
        ...
        if (!force) {
            try {
                ...
                reply.status(400).send({
                    message: "Bayi kaydi bagli veriler nedeniyle silinemedi. Once kayitlari temizleyin veya bagli verilerle silmeyi secin."
                });
            }
            return;
        }
    ...
    app.post("/dealers/:id/integrations", async (request, reply) => {
        ...
        const dealer = await prisma.dealer.findUnique({ where: { id: request.params.id } });
        if (!dealer) {
    ...
    app.delete("/dealers/:dealerId/integrations/:integrationId", async (request, reply) => {
        const dealer = await prisma.dealer.findUnique({ where: { id: request.params.dealerId } });
        if (!dealer) {
            reply.status(404).send({ message: "Dealer not found" });
            return;
        }
        const integration = await prisma.dealerIntegration.findFirst({
            where: {
                id: request.params.integrationId,
                dealerId: request.params.dealerId
            }
        });
        if (!integration) {
    ...
    app.get("/dealer/me", async (request, reply) => {
        ...
        if (!settings)
            return;
        const authUser = request.dealerAuthUser ?? null;
        reply.send({
            dealer: settings.dealer,
            settings: {
                marginPercent: Number(settings.marginPercent ?? 0),
    ...
    app.get("/dealer/customers", async (request, reply) => {
        ...
        const statsByCustomerId = new Map(stats
        ...
        reply.send(customers.map((customer) => {
    ...
    app.get("/dealer/storefront-link", async (request, reply) => {
        ...
        const proto = typeof forwardedProto === "string" && forwardedProto.trim().length
            ? forwardedProto.split(",")[0]
            : request.protocol || "http";
        const host = request.headers.host || "localhost:3000";
        const storefrontUrl = `${proto}://${host}/storefront.html?key=${encodeURIComponent(settings.storefrontKey)}`;
        reply.send({
    ...
    app.get("/storefront/:storefrontKey/catalog", async (request, reply) => {
        const storefrontKey = request.params.storefrontKey;
    ...
    app.post("/dealer/orders/:id/reorder", async (request, reply) => {
        ...
        if (!settings)
            return;
        const dealer = settings.dealer;
        const sourceOrderId = request.params.id;
    ...
    app.post("/dealer/service-requests", async (request, reply) => {
        ...
        const paymentLink = data.type === "PAYMENT_LINK"
