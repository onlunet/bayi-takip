...
import { buildCsv, buildXlsxBuffer, detectFormat, parseSpreadsheetFromBase64, pickNumber, pickString } from "../lib/spreadsheet.js";
...
async function ensureDealerPriceList(dealerId) {
...
export async function registerPriceListRoutes(app) {
    ...
    app.get("/dealers/:id/price-lists/export", async (request, reply) => {
        ...
        const format = detectFormat(request.query?.format);
        ...
        const itemMap = new Map(priceItems.map((item) => [`${item.productId}:${item.variantId ?? ""}`, Number(item.price)]));
        const headers = [
            "productId",
            "variantId",
            "sku",
        ...
        for (const product of products) {
            ...
            for (const variant of product.variants) {
                const variantPrice = itemMap.get(`${product.id}:${variant.id}`) ??
        ...
        if (format === "csv") {
            ...
            reply
                ...
                .header("Content-Disposition", `attachment; filename="bayi-${safeDealer}-${safeList}-fiyat.csv"`)
                .send(csv);
            return;
        }
        ...
        reply
            ...
            .header("Content-Disposition", `attachment; filename="bayi-${safeDealer}-${safeList}-fiyat.xlsx"`)
            .send(buffer);
    });
    app.post("/dealers/:id/price-lists/import", async (request, reply) => {
        ...
        const productBySku = new Map(products
        ...
        const variantBySku = new Map(variants
            .filter((item) => item.sku)
            .map((item) => [String(item.sku).toLocaleLowerCase("tr"), item]));
        const result = await prisma.$transaction(async (tx) => {
            ...
            for (const row of parsedRows) {
                try {
                    ...
                    if (!product && skuInput)
                        product = productBySku.get(skuInput);
                    if (!product && productNameInput)
                        product = productByName.get(productNameInput);
                    if (!product) {
