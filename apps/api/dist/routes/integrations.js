...
async function runIntegrationSyncJob(jobId, authUserId) {
...
export async function registerIntegrationRoutes(app) {
    ...
    app.get("/integrations/jobs", async (request, reply) => {
        const companyId = request.query.companyId;
        const dealerId = request.query.dealerId;
        const integrationId = request.query.integrationId;
    ...
    app.post("/integrations/:id/sync", async (request, reply) => {
        try {
            ...
            const compactOptions = compactObject(syncOptions);
    ...
    app.post("/integrations/jobs/:id/replay", async (request, reply) => {
        ...
        const authUserId = request.authUser?.id ?? null;
        const baseOptions = sourceJob.options && typeof sourceJob.options === "object" && !Array.isArray(sourceJob.options)
            ? sourceJob.options
            : {};
