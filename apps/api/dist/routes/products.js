...
import { buildCsv, buildXlsxBuffer, detectFormat, parseSpreadsheetFromBase64, pickBoolean, pickNumber, pickString } from "../lib/spreadsheet.js";
...
function parseUnit(raw, defaultUnit) {
    ...
    if (["kg", "kilogram", "kilo"].includes(value))
        return "KG";
    if (["lt", "l", "litre", "liter"].includes(value))
        return "LT";
    if (["piece", "adet", "ad", "pcs", "pc"].includes(value))
        return "PIECE";
    return defaultUnit;
}
...
export async function registerProductRoutes(app) {
    ...
    app.post("/products/import", async (request, reply) => {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const categoryMap = new Map(categories.map((category) => [category.name.toLocaleLowerCase("tr"), category.id]));
            let created = 0;
            let updated = 0;
            let skipped = 0;
            const errors = [];
            for (const row of parsedRows) {
                try {
                    ...
                    const unit = parseUnit(pickString(normalized, ["birim", "unit"], payload.defaultUnit), payload.defaultUnit);
                    const weight = pickNumber(normalized, ["gramaj", "agirlik", "weight"], NaN);
                    const basePrice = pickNumber(normalized, ["bazfiyat", "baz_fiyat", "fiyat", "price", "birimfiyat"], NaN);
