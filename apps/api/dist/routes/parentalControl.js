...
function canIssueCode(lastIssuedAt) {
    if (!lastIssuedAt)
        return true;
...
function normalizeParentVerificationCode(raw) {
    if (!raw || typeof raw !== "object")
        return null;
    if (typeof raw.childId !== "string")
        return null;
    const createdAt = typeof raw.createdAt === "string" ? raw.createdAt : nowIso();
    const expiresAt = typeof raw.expiresAt === "string"
        ? raw.expiresAt
...
function normalizeChildRecord(raw) {
    ...
    const passwordSalt = typeof raw?.passwordSalt === "string" ? raw.passwordSalt : hashSecret("child-pass").salt;
    ...
    const sessions = Array.isArray(raw?.sessions)
        ? raw.sessions
            .filter((entry) => typeof entry?.token === "string" && typeof entry?.expiresAt === "string")
            .map((entry) => ({ token: entry.token, expiresAt: entry.expiresAt }))
        : [];
    const createdAt = typeof raw?.createdAt === "string" ? raw.createdAt : nowIso();
    ...
    const childPasswordUpdatedAt = typeof raw?.childPasswordUpdatedAt === "string"
...
function normalizeUsageEvent(raw) {
    if (!raw || typeof raw !== "object")
        return null;
    if (typeof raw.childId !== "string" || typeof raw.appName !== "string")
        return null;
...
function normalizeLocationEvent(raw) {
    if (!raw || typeof raw !== "object")
        return null;
    if (typeof raw.childId !== "string")
        return null;
...
function normalizeFamilyRecord(raw) {
    const createdAt = typeof raw?.parent?.createdAt === "string" ? raw.parent.createdAt : nowIso();
    const parentPinSalt = typeof raw?.parent?.pinSalt === "string" ? raw.parent.pinSalt : hashSecret("1234").salt;
    const parentPinHash = typeof raw?.parent?.pinHash === "string"
        ? raw.parent.pinHash
    ...
    const compliance = {
        privacyPolicyVersion: typeof raw?.compliance?.privacyPolicyVersion === "string"
            ? raw.compliance.privacyPolicyVersion
            : "v1",
        termsVersion: typeof raw?.compliance?.termsVersion === "string" ? raw.compliance.termsVersion : "v1",
        protectionMode: raw?.compliance?.protectionMode === "ENHANCED_FAMILY_LINK"
            ? "ENHANCED_FAMILY_LINK"
            : "STANDARD",
        prominentDisclosureAcceptedAt: typeof raw?.compliance?.prominentDisclosureAcceptedAt === "string"
            ? raw.compliance.prominentDisclosureAcceptedAt
            : createdAt,
        childMonitoringDisclosureAcceptedAt: typeof raw?.compliance?.childMonitoringDisclosureAcceptedAt === "string"
        ...
        dataRetentionDays: typeof raw?.compliance?.dataRetentionDays === "number" &&
            raw.compliance.dataRetentionDays >= 30 &&
            raw.compliance.dataRetentionDays <= 365
            ? Math.floor(raw.compliance.dataRetentionDays)
            : DEFAULT_DATA_RETENTION_DAYS,
        accountDeletionAvailable: typeof raw?.compliance?.accountDeletionAvailable === "boolean"
    ...
    return {
        ...
        parent,
        compliance,
        lastPairingCodeIssuedAt: typeof raw?.lastPairingCodeIssuedAt === "string" ? raw.lastPairingCodeIssuedAt : null,
        lastParentVerificationCodeIssuedAt: typeof raw?.lastParentVerificationCodeIssuedAt === "string"
...
function cleanupExpired(store) {
    ...
    for (const family of store.families) {
        ...
        family.parentVerificationCodes = family.parentVerificationCodes.filter((code) => safeDateMs(code.expiresAt) > now || code.usedAt);
...
function readStoreSnapshot(reader) {
    const store = readStore();
    cleanupExpired(store);
    return reader(store);
}
function findFamilyByParentToken(store, token) {
    return store.families.find((family) => family.parentSessions.some((session) => session.token === token)) ?? null;
}
function findFamilyAndChildByChildToken(store, childId, token) {
    for (const family of store.families) {
...
function sameUtcDate(isoString, date) {
    const parsed = new Date(isoString);
    if (!Number.isFinite(parsed.getTime()))
        return false;
    return parsed.toISOString().slice(0, 10) === date;
}
function summarizeUsage(events, date) {
    const byApp = new Map();
    const byHour = Array.from({ length: 24 }, (_, hour) => ({ hour, minutes: 0 }));
    let totalMinutes = 0;
    for (const event of events) {
        if (!sameUtcDate(event.startedAt, date))
            continue;
...
function getLatestLocationEvent(events, childId) {
    let latest = null;
    let latestMs = 0;
    for (const event of events) {
...
function summarizeLocationHistory(events, childId, date, limit) {
...
function toSafeUtcDateKey(value) {
    const parsed = new Date(value);
    if (!Number.isFinite(parsed.getTime()))
        return null;
    return parsed.toISOString().slice(0, 10);
}
function buildRecentDateKeys(endDate, days) {
...
function isWithinLastHours(isoDate, hours) {
...
function getParentalAdminKeyFromRequest(request) {
    const parentKey = request.headers["x-parental-admin-key"];
    if (parentKey)
        return Array.isArray(parentKey) ? parentKey[0] : parentKey;
    const generic = request.headers["x-admin-key"];
    if (!generic)
        return null;
    return Array.isArray(generic) ? generic[0] : generic;
}
function authorizeParentalAdminRequest(request) {
    ...
    if (!suppliedKey || suppliedKey !== configuredKey) {
        return { ok: false, statusCode: 401, message: "x-parental-admin-key gerekli veya gecersiz" };
    }
    return { ok: true, insecureMode: false };
}
function buildAdminOverview(store, endDate, days) {
    ...
    const families = store.families
        ...
        const activeChildren24h = family.children.filter((child) => isWithinLastHours(child.lastSeenAt, ACTIVE_CHILD_WINDOW_HOURS)).length;
    ...
    const activeChildren24h = allChildren.filter(({ child }) => isWithinLastHours(child.lastSeenAt, ACTIVE_CHILD_WINDOW_HOURS)).length;
    const parentSessionsActive = store.families.reduce((total, family) => total + family.parentSessions.length, 0);
    const enhancedFamilies = store.families.filter((family) => family.compliance.protectionMode === "ENHANCED_FAMILY_LINK").length;
...
function getParentToken(request) {
    const header = request.headers["x-parent-token"];
    if (!header)
        return null;
    return Array.isArray(header) ? header[0] : header;
}
function getChildToken(request) {
    const header = request.headers["x-child-token"];
    if (!header)
        return null;
    return Array.isArray(header) ? header[0] : header;
}
function buildComplianceStatus(family) {
...
export async function registerParentalControlRoutes(app) {
    ...
    app.post("/parental-control/parent/pairing-code", async (request, reply) => {
        ...
        const result = mutateStore((store) => {
            ...
            if (!family)
                return { error: "Gecersiz parent token" };
            if (!canIssueCode(family.lastPairingCodeIssuedAt)) {
                return { error: "Cok sik kod uretiliyor. Lutfen kisa bir sure bekleyin." };
            }
            const now = new Date();
    ...
    app.put("/parental-control/children/:childId/password", async (request, reply) => {
        ...
        const result = mutateStore((store) => {
            ...
            const child = family.children.find((item) => item.id === childId);
            if (!child)
                return { error: "Cocuk bulunamadi", statusCode: 404 };
    ...
    app.post("/parental-control/children/login", async (request, reply) => {
        ...
        const result = mutateStore((store) => {
            ...
            if (!child)
                return { error: "Cocuk hesabi bulunamadi" };
            if (child.deviceId !== data.deviceId) {
                return { error: "Cihaz kimligi eslesmiyor" };
            }
            if (!child.childPasswordUpdatedAt) {
                return {
                    error: "Ebeveyn bu cocuk icin giris sifresini henuz belirlemedi",
                    code: "PARENT_PASSWORD_REQUIRED"
                };
            }
    ...
    app.post("/parental-control/children/:childId/ack-disclosure", async (request, reply) => {
        ...
        const result = mutateStore((store) => {
            ...
            if (!identity)
                return { error: "Yetkisiz child token" };
            const now = nowIso();
    ...
    app.post("/parental-control/children/:childId/verify-parent-code", async (request, reply) => {
        ...
        const result = mutateStore((store) => {
            const identity = findFamilyAndChildByChildToken(store, childId, childToken);
            if (!identity)
                return { error: "Gecersiz child token" };
            const codeEntry = identity.family.parentVerificationCodes.find((entry) => entry.childId === childId &&
                !entry.usedAt &&
                safeDateMs(entry.expiresAt) > Date.now() &&
    ...
    app.get("/parental-control/children/:childId/policy", async (request, reply) => {
        ...
        const result = readStoreSnapshot((store) => {
            ...
            if (childToken) {
                const identity = findFamilyAndChildByChildToken(store, childId, childToken);
                if (identity)
                    return { policy: identity.child.policy };
            }
    ...
    app.post("/parental-control/children/:childId/usage", async (request, reply) => {
        ...
        const result = mutateStore((store) => {
            ...
            if (!identity.child.disclosureAckAt) {
                return { error: "Cocuk bilgilendirme onayi gerekli", code: "DISCLOSURE_REQUIRED" };
            }
    ...
    app.post("/parental-control/children/:childId/location", async (request, reply) => {
        ...
        const result = mutateStore((store) => {
            ...
            const event = {
                ...
                accuracyMeters: typeof parsed.data.accuracyMeters === "number"
    ...
    app.get("/parental-control/children/:childId/usage-summary", async (request, reply) => {
        ...
        const childId = request.params.childId;
        const queryDate = request.query.date ?? nowIso().slice(0, 10);
    ...
    app.get("/parental-control/parent/dashboard", async (request, reply) => {
        ...
        const result = readStoreSnapshot((store) => {
            ...
            const children = family.children.map((child) => {
                const summary = summarizeUsage(family.usageEvents.filter((event) => event.childId === child.id), queryDate);
