...
export function canAccessAdminRoute(role, method, rawPath) {
    ...
    if (role === "DEALER")
        return false;
    if (role === "WAREHOUSE") {
        if (pathMatchesAny(path, WAREHOUSE_DENIED_PREFIXES))
            return false;
...
export function isPublicPath(rawPath) {
    const path = normalizePath(rawPath);
...
export function isDealerPath(rawPath) {
    const path = normalizePath(rawPath);
    if (path === "/catalog.xml")
        return true;
